...
function restoreOptions() {
    ...
    $('#custom_icon_data').prop(
        'value',
        localStorage.getItem('custom_icon_data'),
    );
    $('#center_vertically').prop(
        'checked',
        localStorage.getItem('center_vertically') === 'true',
    );
    ...
    $('#enable_sync').prop(
        'checked',
        localStorage.getItem('enable_sync') === 'true',
    );
    $('#folder_color').prop('value', localStorage.getItem('folder_color'));
    $('#force_http').prop(
        'checked',
        localStorage.getItem('force_http') === 'true',
    );
    $('#show_advanced').prop(
        'checked',
        localStorage.getItem('show_advanced') === 'true',
    );
    $('#show_folder_list').prop(
        'checked',
        localStorage.getItem('show_folder_list') === 'true',
    );
    $('#show_new_entry').prop(
        'checked',
        localStorage.getItem('show_new_entry') === 'true',
    );
    $('#show_options_gear').prop(
        'checked',
        localStorage.getItem('show_options_gear') === 'true',
    );
    $('#show_subfolder_icons').prop(
        'checked',
        localStorage.getItem('show_subfolder_icons') === 'true',
    );
    $('#thumbnailing_service').prop(
        'value',
        localStorage.getItem('thumbnailing_service'),
    );
}
...
function saveOptions() {
    localStorage.setItem(
        'background_color',
        $('#background_color').prop('value'),
    );
    localStorage.setItem(
        'custom_icon_data',
        JSON.stringify(JSON.parse($('#custom_icon_data').prop('value'))),
    );
    localStorage.setItem(
        'center_vertically',
        $('#center_vertically').prop('checked'),
    );
    ...
    localStorage.setItem(
        'show_folder_list',
        $('#show_folder_list').prop('checked'),
    );
    localStorage.setItem(
        'show_new_entry',
        $('#show_new_entry').prop('checked'),
    );
    localStorage.setItem(
        'show_options_gear',
        $('#show_options_gear').prop('checked'),
    );
    localStorage.setItem(
        'show_subfolder_icons',
        $('#show_subfolder_icons').prop('checked'),
    );
    localStorage.setItem(
        'thumbnailing_service',
        $('#thumbnailing_service').prop('value'),
    );
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#save').on('click', function () {
        try {
            // Just validate and make sure everything is good to save
            JSON.parse($('#custom_icon_data').prop('value'));
            saveOptions();
        } catch (e) {
            alert(
                'The JSON you entered is not valid, try again.\nThe error message was: ' +
                    e,
            );
        }
    });
    ...
    $(document.body).on('keyup', function (e) {
        if (e.which === 27) {
            // 27 is the character code of the escape key
            window.location = 'newtab.html';
        }
    });
        ...
        if (this.checked) {
            chrome.storage.sync.getBytesInUse(null, function (bytes) {
                if (bytes > 0) {
                    if (
                        confirm(
                            'You have previously synchronized data!!\n' +
    ...
    $('#custom_icon_data').on('keydown', function (e) {
        if (e.which === 13) {
            // 13 is the character code of the return key
            $('#save').trigger('click');
        }
    });
    ...
    $('#reset_local_settings').on('click', function () {
        if (
            confirm(
                'Are you sure you want to reset all local values you have changed back to their defaults?',
            )
        ) {
            localStorage.clear();
            createDefaults();
    ...
    $('#reset_synced_settings').on('click', function () {
        if (
            confirm(
                'Are you sure you want to delete all previously synchronized data to start fresh?',
            )
        ) {
