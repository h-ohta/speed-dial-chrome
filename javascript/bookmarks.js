...
function isValidUrl(url) {
    ...
    var URL_REGEXP =
        /^(http|https|ftp|file|chrome|chrome-extension):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?$/;
...
function updateBookmark(id, title, url) {
    ...
    if (hash !== undefined) {
        chrome.bookmarks.update(id, hash, function (result) {
            ...
            const use_thumbnail_checked = $('#bookmark_form .thumbnail').prop(
                'checked',
            );
        ...
        alert(
            'Editing an existing Bookmark requires both a Title and a valid URL in Chrome\n\n' +
