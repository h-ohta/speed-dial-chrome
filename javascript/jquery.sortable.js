...
(function ($) {
    ...
    $.fn.sortable = function (options) {
        ...
        return this.each(function () {
            ...
            var placeholder = $('<div>').prop(
                'class',
                'entry sortable-placeholder',
            );
            items
                .find(options.handle)
                .mousedown(function () {
            ...
            items
                ...
                .on('dragstart.h5s', function (e) {
                    ...
                    // *modified* to set the drag & drop ghost image to the image of the entry image
                    var target = this.firstChild.firstChild;
                    dt.setDragImage(
                        target,
                ...
                .on('dragover.h5s dragenter.h5s drop.h5s', function (e) {
                    if (
                        !items.is(dragging) &&
                        options.connectWith !==
                            $(dragging).parent().data('connectWith')
                    ) {
                        return true;
                    }
                    if (e.type == 'drop') {
                    ...
                    if (items.is(this)) {
                        if (options.forcePlaceholderSize) {
                            ...
                            placeholder.height(dragging.outerWidth());
                        }
                        dragging.hide();
                        $(this)[
