...
function addSpeedDialBookmark(bookmark, entryArray) {
    ...
    entry
        .find('.image')
        .css('background-image', 'url(' + getThumbnailUrl(bookmark) + ')');
    entry.find('.edit').on('click', function (event) {
        event.preventDefault();
        showBookmarkEntryForm(
...
function addSpeedDialFolder(bookmark, entryArray) {
    var entry = $(
        '<div class="entry" id="' +
            bookmark.id +
            '">' +
            '<a class="bookmark" href="newtab.html#' +
            bookmark.id +
            '" title="' +
            bookmark.title +
            '" >' +
    ...
    entry.find('.remove').on('click', function (event) {
        ...
        if (
            confirm(
                "Are you sure you want to remove this folder including all of it's bookmarks?",
            )
        ) {
...
function setDialStyles() {
    ...
    $('#styles').html(
        '#dial { width:' +
            ...
            'px; } ' +
            '.image { height:' +
            ((entryWidth * 0.75 - 20) | 0) +
            'px; } ' +
            '.foundicon-folder { font-size:' +
            ((entryWidth * 0.5) | 0) +
            'px; top:' +
            ((entryWidth * 0.05) | 0) +
            'px; color:' +
...
function createSpeedDial(folderId) {
    ...
    chrome.bookmarks.getSubTree(folderId, function (node) {
        ...
        if (
            ...
            $('#options').append(
                $(
                    '<a class="foundicon-settings" href="options.html" title="Options"></a>',
                ),
            );
        }
...
function getThumbnailUrl(bookmark) {
    if (JSON.parse(localStorage.getItem('custom_icon_data'))[bookmark.url]) {
        return JSON.parse(localStorage.getItem('custom_icon_data'))[
            bookmark.url
        ];
    }

    const use_thumbnail = JSON.parse(
        localStorage.getItem('use_thumbnail') || '{}',
    );
    if (use_thumbnail[bookmark.url] === true) {
        ...
        return localStorage
            .getItem('thumbnailing_service')
            .replace('[URL]', urlForThumbnail);
    }
...
function showBookmarkEntryForm(heading, title, url, target) {
    ...
    form.find('.icon').prop(
        'value',
        JSON.parse(localStorage.getItem('custom_icon_data'))[url] || '',
    );
...
function alignVertical() {
    if (localStorage.getItem('center_vertically') === 'true') {
        ...
        if (localStorage.getItem('show_folder_list') === 'true') {
            dial.css(
                'padding-top',
                ((window.innerHeight - dial.height()) / 2 - 50) | 0,
            );
        } else {
            dial.css(
                'padding-top',
                ((window.innerHeight - dial.height()) / 2) | 0,
            );
        }
    }
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('#bookmark_form .title, #bookmark_form .url, #bookmark_form .icon').on(
        'keydown',
        function (e) {
            if (e.which === 13) {
